...
public class Map :MonoBehaviour
{
    ...
    {
        if (index == 1)
            return true; // The first restaurant is always unlocked

        // Load the previous restaurant's data to check if the current restaurant is unlocked
        var previousRestaurantData = SaveSystem.LoadData<StageData>($"Stage{index + 1}");

        if (previousRestaurantData == null)
            return false; // If no data is found, the restaurant is locked
